...
class NLP {
    ...
    _generateTerrain(scene, type) {
        switch(type.toLowerCase()) {
            case "forest": for(let i=0;i<20;i++){ const tree=`Tree${i}`; this.graph.addNode(scene,tree,"StaticBody","World"); this.graph.addScript(`${tree}_script.gd`,`extends StaticBody\n# Tree collision`); this.graph.attachScript(scene,tree,`${tree}_script.gd`);} break;
            case "dungeon": for(let i=0;i<50;i++){ const tile=`DungeonTile${i}`; this.graph.addNode(scene,tile,"StaticBody","World");} break;
            case "open field": for(let i=0;i<10;i++){ const rock=`Rock${i}`; this.graph.addNode(scene,rock,"StaticBody","World");} break;
        }
    }
